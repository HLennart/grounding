...
impl State {
    async fn new(window: &Window) -> Self {
        ...
        world.push((Position { x: 1.0, y: 1.0 }, Velocity { dx: 0.1, dy: 0.1 }));

        let mut schedules: Vec<Schedule> = Vec::new();
        let pos_schedule = Schedule::builder()
            .add_system(update_positions_system())
            .build();

        schedules.push(pos_schedule);
        ...
        match event {
            WindowEvent::CursorMoved { position, .. } => {
                self.clear_color = wgpu::Color {
                    ...
                    g: position.x as f64 / self.size.height as f64,
                    b: 1.0,
                    a: 1.0,
                };
                true
    ...
    fn update(&mut self) {
        for s in &mut self.schedules {
            s.execute(&mut self.world, &mut Resources::default());
        }
    }
